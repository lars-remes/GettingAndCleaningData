...
* Reads the raw data files (train/X_train.txt, test/X_test.txt)
* Labels the data set with the variable names (features.txt)
* Merges activity and subject ids to data set (y_test.txt, y_train.txt, subject_test.txt, subject_train.txt)
* Converts activity ids to factor variable and labels the activities (activity_labels.txt)
* Merges the labelled 'train' and 'test' data sets to create one data set.
* Extracts - using select() - only the measurements on the mean (mean()) and standard deviation (std()) for each measurement. 
...
The original study and data set has been described <a href=”http://archive.ics.uci.edu/ml/datasets/Human+Activity+Recognition+Using+Smartphones”>here</a>.
